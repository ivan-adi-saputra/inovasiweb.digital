...
export default function List() {
  ...
  const session = useSelector((state: RootState) => state.auth.token);
  console.log("session");
  console.log(session);
      ...
      <div className="mt-12 lg:mt-0">
        {session ? (
          <button
            ...
            className="relative flex h-9 w-full items-center justify-center px-6 before:absolute before:inset-0 before:rounded-full before:bg-purple-500 before:transition before:duration-300 hover:before:scale-105 active:duration-75 active:before:scale-95 sm:w-max"
          >
            <span className="relative text-md text-white">Logout</span>
          </button>
        ) : (
          <Link
            href="/auth/signin"
